...
10. **Trusted client-IP attribution** — `CF-Connecting-IP` / `X-Forwarded-For` are honoured only when the direct peer is inside `TUSKER_TRUSTED_PROXY_RANGES` (Cloudflare ranges by default); any other caller cannot spoof logged or audited IPs.
11. **Guardrail precision** — PII redaction replaces emails and Luhn-valid card numbers only (long digit runs like order IDs pass through), and prompt-injection patterns match message-initial directives in user turns; quoted or described phrases elsewhere pass.
