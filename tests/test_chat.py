...
@pytest.mark.asyncio
async def test_chat_stream_malformed_tool_markup_after_terminal_falls_back_before_client_response(app, client):
    """Trailing malformed tool text must be rejected before a 200 is sent."""
    pool_manager = MagicMock()
    pool_manager.select.side_effect = [
        ("openrouter", "nvidia/terminal-then-malformed-model"),
        ("openai", "tool-capable-fallback"),
    ]
    app["pool_manager"] = pool_manager
    ...
    async def malformed_stream(*args, **kwargs):
        yield (
            b'data: {"choices":[{"index":0,"delta":{"role":"assistant",'
            b'"content":"I will inspect the manifests."},"finish_reason":"stop"}]}'
            b'\n\n'
        )
        yield (
            b'data: {"choices":[{"index":0,"delta":{"content":" <tool_call>find '
            b'/Volumes/dev/dev/k8s -name \\\"*.yaml\\\" -exec grep -l \\\"ConfigMap\\\" {} '
            b'\\\\; </parameter></function></tool_call>"},"finish_reason":null}]}'
            b'\n\n'
        )
